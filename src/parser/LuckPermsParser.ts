...
type LuckPermsVersionItem = Omit<
  LuckPermsVersionItemWithDownloadUrl,
  'downloadUrl'
>
...
export class LuckPermsParser extends BaseParser<
  BaseParserOptions,
  LuckPermsResult
> {
  constructor() {
  ...
  async parse(): Promise<LuckPermsResult> {
    ...
    if (response.status !== 200) {
      throw new ParseError('Failed to get latest version of LuckPerms')
    }
    const latest = {
    ...
    const items: LuckPermsVersionItem[] = response.data.changeLog.map(
      (item) => ({
