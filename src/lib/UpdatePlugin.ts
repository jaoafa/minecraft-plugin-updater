...
export default async function (plugin: IPlugin, result: ParserResults, serverVersion: string) {
  const filename = `plugins/${plugin.name}-${result.latest.version}.jar`
  switch (plugin.package.constructor.name) {
    case 'BukkitParser': {
      const item = (result as BukkitResult).versions.find(v => v.gameVersions.includes(serverVersion))
      if (!item) {
    ...
    case 'GitHubReleaseParser':
      await download((result as GitHubReleaseResult).latest.downloadUrl, filename)
      break
    case 'JenkinsBuildParser':
      await download((result as JenkinsBuildResult).latest.downloadUrl, filename)
      break
    case 'SpigotMCParser':
